...
from src.phase4 import SecurityContext, DocumentStore, OperationalDataStore, ActionGateway
from src.tool_dispatcher import ToolDispatcher
from src.llm_provider import GeminiProvider, ToolCall, LLMResponse
...
class LiveToolCallingAgent:
    """
    Production-grade LLM Agent using Google Gemini Tool Calling.
    Executes a bounded reasoning & tool execution loop.
    """
    ...
    def execute_turn(self, prompt: str, context: SecurityContext) -> Dict[str, Any]:
        ...
        system_instruction = (
            "You are the ParcelPilot AI Support Agent. You assist support staff and customers with order cancellations, "
            "service credit inquiries, and SLA escalations. You MUST select and invoke appropriate tools to look up facts "
        ...
        while iteration < MAX_TOOL_ITERATIONS:
            ...
            # If LLM generated a final text response without new tool calls
    ...
    def _build_final_response(self, text: Optional[str], tool_trace: List[Dict[str, Any]], context: SecurityContext) -> Dict[str, Any]:
        ...
        # Integrate deterministic decision if available
...
class DeterministicToolEngine:
    """
    Deterministic Tool Engine for offline test suites, CI, and evaluation without live API keys.
    Explicitly labeled as an offline test fixture.
    """
...
class AgentService:
    """
    Master Service exposing agent capability to clients (Streamlit UI, APIs, Tests).
    Auto-detects whether Live Gemini Agent or Offline Deterministic Engine should be active.
    """
